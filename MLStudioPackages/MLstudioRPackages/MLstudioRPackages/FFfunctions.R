...
ConvertFanduelCSV <- function(t,id,w){
...
OptimizeFBLineup <- function(dt){
  RunOptimizer(dt)#(dt[!is.na(data$Rk)])
}
...
GetRushDForMLSudio <- function(rd){
  ...
  names(rd)[names(rd)=="Yds"] <- "RushYdsAllowed"
  names(rd)[names(rd)=="Avg"] <- "AverageYardsPerCarryAllowed"
  names(rd)[names(rd)=="Yd/.G"] <- "RushingYardsPerGameAllowed"
  names(rd)[names(rd)=="TD"] <- "RushingTDAllowed"
  names(rd)[names(rd)=="Lng"] <- "LongestRunAllowed"
  names(rd)[names(rd)=="Avg"] <- "AverageYardsPerCarryAllowed"
  names(rd)[names(rd)=="20+"] <- "20yardsRunOrLongerAllowed"
  names(rd)[names(rd)=="40+"] <- "40yardsRunOrLongerAllowed"
  names(rd)[names(rd)=="FUM"] <- "FumblesCaused"
  names(rd)[names(rd)=="1st"] <- "FirstDownsAllowed"
  names(rd)[names(rd)=="1st%"] <- "FirstDownPercentageAllowed"
  names(rd)[names(rd)=="G"] <- "RushingGamesPlayed"
  
  return (rd)
}

GetPassDForMLStudio <- function(pd){
  ...
  names(pd)[names(pd)=="Lng"] <- "LongestPassAllowed"
  names(pd)[names(pd)=="Avg"] <- "AveragePassYardsAllowed"
  names(pd)[names(pd)=="20+"] <- "20yardsPassOrLongerAllowed"
  names(pd)[names(pd)=="40+"] <- "40yardsPassOrLongerAllowed"
  names(pd)[names(pd)=="Sck"] <- "Sacs"
  names(pd)[names(pd)=="1st"] <- "PassingFirstDownsAllowed"
  names(pd)[names(pd)=="1st%"] <- "PassingFirstDownPercentageAllowed"
  names(pd)[names(pd)=="G"] <- "PassingGamesPlayed"
  names(pd)[names(pd)=="Rate"] <- "PassDefenseRating"
  names(pd)[names(pd)=="Comp"] <- "CompletionsAllowed"
  names(pd)[names(pd)=="Pct"] <- "CompletionPercentageAllowed"
...
GetPlayerstable <- function(x,names){
