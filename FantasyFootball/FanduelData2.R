...
path <- 'C:/Users/Owner/OneDrive/Source/Repos/R/FantasyFootball/Fanduel/FanDuel-NFL-2016-10-13-16564-players-list.csv'
name <- 'W5_ThusMon.csv'
...
players <- players[which(players$Name %like% "Defense" == FALSE)]
...
setkey(players,Name)
finalfandueltable <- merge(final,players,all = TRUE)


finalfandueltable <- finalfandueltable[which(!is.na(finalfandueltable$Year)), ]


names(finalfandueltable)[names(finalfandueltable)=="TeamID.x"] <- "TeamID"


setkey(finalfandueltable,TeamID)
setkey(d,TeamID)


finalfinal1 <- merge(finalfandueltable,d,all = TRUE)
#Assign 0 to NAs
finalfinal1[is.na(finalfinal1)] <- 0


# stats <- final
# #Upload data for prediction
UploadToMLStudio(finalfinal1,'W5_ThursMon1.csv')
